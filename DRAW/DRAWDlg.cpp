...
BEGIN_MESSAGE_MAP(CDRAWDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_UNDO, &CDRAWDlg::OnBnClickedUndo)
	ON_BN_CLICKED(IDC_REDO, &CDRAWDlg::OnBnClickedRedo)
...
void CDRAWDlg::Draw(CDC* pDC) {
	...
	if (m_bDrawingShape && m_LButtonDown) {
		...
		case ShapeType::CIRCLE: {
			...
			pDrawDC->Ellipse(static_cast<int>(ptStart.x - r), static_cast<int>(ptStart.y - r), static_cast<int>(ptStart.x + r), static_cast<int>(ptStart.y + r));
			break;
		}
		default: break;
		}
	...
	for (INT_PTR i = 0; i < m_SelectedPoints.GetSize(); i++) {
		SPoint* pPoint = static_cast<SPoint*>(m_SelectedPoints.GetAt(i));
		if (pPoint) {
			SPoint screenPoint = *pPoint;
			SPoint::xy2XY(screenPoint, SPoint(0, (double)m_x1, (double)m_y2), m_StateShow);
			const double x = screenPoint.m_x + m_StateShow.m_dx;
			const double y = screenPoint.m_y + m_StateShow.m_dy;
			const int radius = 4;
			pDrawDC->Ellipse(static_cast<int>(x - radius), static_cast<int>(y - radius), static_cast<int>(x + radius), static_cast<int>(y + radius));
		}
	}
	pDrawDC->SelectObject(pOldPen);
...
void CDRAWDlg::OnBnClickedOpen() {
	...
	if (dlg.DoModal() == IDOK) { m_csFilePath = dlg.GetPathName(); m_Data.ReadPlt(m_csFilePath); GetDlgItem(IDC_SAVE)->EnableWindow(true); Invalidate(); UpdateUndoRedoButtons(); }
}
	...
	if (m_LButtonDown) {
		...
		if (m_bDraw) {
			...
			if (m_pMovingPoint) {
				...
				worldPoint.m_x -= m_StateShow.m_dx; worldPoint.m_y -= m_StateShow.m_dy;
				SPoint::XY2xy(worldPoint, SPoint(0, (double)m_x1, (double)m_y2), m_StateShow);
				m_pMovingPoint->m_x = worldPoint.m_x; m_pMovingPoint->m_y = worldPoint.m_y;
			}
...
void CDRAWDlg::OnBnClickedNew() { m_Data.Clear(); Invalidate(); UpdateUndoRedoButtons(); }
...
void CDRAWDlg::OnBnClickedUp() { m_StateShow.m_dy += 100 / m_StateShow.m_r; Invalidate(); }
void CDRAWDlg::OnBnClickedDown() { m_StateShow.m_dy -= 100 / m_StateShow.m_r; Invalidate(); }
void CDRAWDlg::OnBnClickedMl() { m_StateShow.m_dx += 100 / m_StateShow.m_r; Invalidate(); }
void CDRAWDlg::OnBnClickedMr() { m_StateShow.m_dx -= 100 / m_StateShow.m_r; Invalidate(); }
...
void CDRAWDlg::OnBnClickedDelPoint() {
	if (m_SelectedPoints.GetSize() > 0) {
		for (INT_PTR i = 0; i < m_SelectedPoints.GetSize(); i++) {
			if (SPoint* pPointToDelete = static_cast<SPoint*>(m_SelectedPoints.GetAt(i))) {
				m_Data.DeletePointAt(*pPointToDelete);
			}
		}
...
void CDRAWDlg::OnBnClickedUndo() { m_Data.Undo(); UpdateUndoRedoButtons(); Invalidate(); }
void CDRAWDlg::OnBnClickedRedo() { m_Data.Redo(); UpdateUndoRedoButtons(); Invalidate(); }
...
void CDRAWDlg::UpdateUndoRedoButtons()
{
	GetDlgItem(IDC_UNDO)->EnableWindow(m_Data.CanUndo());
	GetDlgItem(IDC_REDO)->EnableWindow(m_Data.CanRedo());
}
